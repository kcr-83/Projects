    ...
    public static class EvalM
    {
        ...
        internal static MalType EvalAst(MalType mal, Env env) =>
            mal switch
            {
                ...
                Map map => map with { Value = MapM.MapFrom(map.Value.Items.Select(kv => (kv.Key, Eval(kv.Value, env)))) },
                _ => mal
            };
        ...
        internal static MalType ApplyLet(LList<MalType>? items, Env env)
            => items switch
            {
                (List { Items: var Bindings }, (var Expr, null)) =>
                    ApplyBindings(Bindings, new Env(MapM.MapFrom<Symbol, MalType>(null), env)).Pipe(newEnv => Eval(Expr, newEnv)),
                _ => throw new Exception($"'let*' requires 2 arguments where the first is a list of bindings and the second is a Mal expression, but got '{items.JoinWithSeparator()}'")
            };
        ...
        internal static Env ApplyBindings(LList<MalType>? items, Env env)
            => items switch
            {
                ...
                _ => throw new Exception($"Bindings argument in let* should contain an even number of elements where even element must be a 'symbol', but got '{items.JoinWithSeparator()}'")
            };
        ...
        internal static MalType ApplyDef(LList<MalType>? items, Env env)
            => items switch
            {
                ...
                _ => throw new Exception($"'def!' requires 2 arguments where the first argument must be 'symbol', but got '{items.JoinWithSeparator()}'")
            };
        ...
        internal static LList<(Symbol, MalType)>? BindFunctionArguments(LList<MalType>? names, LList<MalType>? values)
            ...
            {
                (null, _) => null,
                ((Symbol { Name: "&" }, (Symbol ArgName, null)), var ArgValue) => new((ArgName, new List(ArgValue, ListType.List)), null),
                ...
                _ => throw new Exception($"Cannot bind function arguments, names: '{names.JoinWithSeparator()}' , values: '{values.JoinWithSeparator()}'")
            };
        ...
        internal static MalType ApplyQuote(LList<MalType>? items, Env env)
             => items switch
             {
                 ...
                 _ => throw new Exception($"'quote' requires one argument, but got '{items.JoinWithSeparator()}'")
             };
        ...
        internal static MalType ApplyQuasiquote(LList<MalType>? items, Env env)
             => items switch
             {
                 ...
                 _ => throw new Exception($"'quasiquote' requires one argument, but got '{items.JoinWithSeparator()}'")
             };
        ...
        internal static MalType ApplyQuasiquoteExpand(LList<MalType>? items, Env env)
            => items switch
            {
                ...
                _ => throw new Exception($"'quasiquoteexpand' requires one argument, but got '{items.JoinWithSeparator()}'")
            };
        ...
        internal static MalType TransformQuasiquote(MalType mal)
            => mal switch
            {
                List { Items: (Symbol("unquote"), (var UnquotedMal, null)) } => UnquotedMal,
                List { Items: null } => new List(null, ListType.List),
                List { Items: (var Head, var Tail) } => Head switch
                {
                    List { Items: (Symbol("splice-unquote"), (var UnquotedMal, null)) } =>
                        new List(LListM.LListFrom(new Symbol("concat"), UnquotedMal, TransformQuasiquote(new List(Tail, ListType.List))), ListType.List),
                    _ =>
                        new List(LListM.LListFrom(new Symbol("cons"), TransformQuasiquote(Head), TransformQuasiquote(new List(Tail, ListType.List))), ListType.List),
                },
                Map or Symbol => new List(LListM.LListFrom(new Symbol("quote"), mal), ListType.List),
                _ => mal
            };
        ...
        internal static MalType ApplyDefMacro(LList<MalType>? items, Env env)
            => items switch
            {
                ...
                _ => throw new Exception($"'defmacro!' requires 2 arguments where the first argument must be of type 'symbol' and the second of type 'fn', but got '{items.JoinWithSeparator()}'")
            };
        ...
        internal static MalType ApplyMacroExpand(LList<MalType>? items, Env env)
            => items switch
            {
                ...
                _ => throw new Exception($"'macroexpand' requires 1 argument, but got '{items.JoinWithSeparator()}'")
            };
    }
}